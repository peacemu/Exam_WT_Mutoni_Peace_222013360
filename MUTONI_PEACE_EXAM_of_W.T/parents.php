...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO parents(Parent_ID, Name, Email, Phone) VALUES (?, ?, ?, ? )");
    $stmt->bind_param("isss", $Parent_ID, $Name, $Email, $Phone);
...
<body>
    <center><h2>Parents Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all parents
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $Parent_ID = $row['Parent_ID']; // Fetch the Parent_ID
        echo "<tr>
            <td>" . $row['Parent_ID'] . "</td>
            <td>" . $row['Name'] . "</td>
            <td>" . $row['Email'] . "</td>
            <td>" . $row['Phone'] . "</td>
            <td><a style='padding:4px' href='delete_messages.php?Parent_ID=$Parent_ID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_messages.php?Parent_ID=$Parent_ID'>Update</a></td> 
        </tr>";
    }
