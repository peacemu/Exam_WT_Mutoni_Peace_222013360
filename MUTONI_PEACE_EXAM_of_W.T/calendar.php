...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO calendar(Event_ID, Event_Name, Description, Start_Date, End_Date, Parent_ID, Teacher_ID) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isssssi", $Event_ID, $Event_Name, $Description, $Start_Date, $End_Date, $Parent_ID, $Teacher_ID);
...
<body>
    <center><h2>Calendar Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all calendar
$sql = "SELECT * FROM calendar";
$result = $connection->query($sql);

// Check if there are any calendar
if ($result->num_rows > 0) {
    // Output data for each row
    while ($row = $result->fetch_assoc()) {
        $Event_ID = $row['Event_ID']; // Fetch the Event_ID
        echo "<tr>
            ...
            <td><a style='padding:4px' href='delete_calendar.php?Event_ID=$Event_ID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_calendar.php?Event_ID=$Event_ID'>Update</a></td> 
        </tr>";
    }
